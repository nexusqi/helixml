...
impl CpuTensor {
    ...
    pub fn from_slice(data: &[f32], shape: Shape, dtype: DType, device: &Device) -> Result<Self> {
        let arr = ArrayD::from_shape_vec(IxDyn(shape.as_slice()), data.to_vec())
            .map_err(|_| TensorError::BackendError {
                message: "Failed to create tensor from slice".to_string(),
            })?;
...
impl TensorOps for CpuTensor {
    ...
    fn from_scalar(value: f32, shape: Shape, dtype: DType, device: &Device) -> Result<Self> {
        ...
        let data = ArrayD::from_elem(IxDyn(shape.as_slice()), value);
    ...
    fn gt(&self, other: &Self) -> Result<Self> {
        ...
        let mut result_data = Vec::new();
        for (a, b) in self.data.iter().zip(other.data.iter()) {
            result_data.push(if a > b { 1.0 } else { 0.0 });
        }
        
        Ok(Self {
            data: ArrayD::from_shape_vec(self.data.shape(), result_data).map_err(|_| TensorError::BackendError {
                message: "Failed to create comparison result tensor".to_string(),
            })?,
            shape: self.shape.clone(),
            dtype: self.dtype,
            device: self.device.clone(),
    ...
    fn gt_scalar(&self, threshold: f32) -> Result<Self> {
        Ok(Self {
            data: self.data.mapv(|x| if x > threshold { 1.0 } else { 0.0 }),
            shape: self.shape.clone(),
            dtype: self.dtype,
            device: self.device.clone(),
        })
    ...
    fn to_scalar(&self) -> Result<f32> {
        if self.shape.numel() == 1 {
            ...
            Err(TensorError::InvalidInput {
                message: format!("Cannot convert tensor with {} elements to scalar", self.shape.numel()),
            })
        }
    }
}
